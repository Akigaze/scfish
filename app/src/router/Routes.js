...
import {adminRouterConfig, publicRouterConfig,managerRouterConfig} from "./router.config";
      ...
      <Router history={history}>
        <Suspense
          ...
          <Switch>
            ...
            {
              managerRouterConfig.map(route => {
                const Layout = route.layout || Fragment
                return (
                  <Route key={route.name}
                         path={route.path}
                         exact={true}
                         component={() => !(this.props.isAuth&&this.props.isManager)
                           ? <Redirect to="/post"/>
                           : <Layout>
                             <route.component/>
                           </Layout>
...
function mapStateToProps(state, props) {
  return {
    ...
    isManager: state.manager.isManager
  }
}
